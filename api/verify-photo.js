...
module.exports = async function handler(req, res) {
  ...
  if (hasRef) {
    ...
    parts.push({ text: `The FIRST image is a photo just taken by a renter. The SECOND image is the reference photo the owner uploaded when listing their item (a bike, e-bike, scooter, e-scooter, rollerskates, skateboard, or similar ride-share item).
...
MATCH: YES or NO (do the two images show the SAME physical item, allowing for a different angle, lighting, or background? If VALID is NO, answer MATCH as NO.)` });
    ...
    parts.push({ text: 'Does this photo clearly show a real bike, e-bike, scooter, e-scooter, rollerskates, skateboard, or similar ride-share item (or a person with one)? Reply with exactly one word: YES or NO.' });
  }
  ...
  try {
    ...
    if (hasRef) {
      ...
      return res.status(200).json({
        skipped: false,
        isValidVehicle: validMatch ? validMatch[1] === 'YES' : true,
        matchesListing: matchMatch ? matchMatch[1] === 'YES' : true
      });
    }
